...
const ALLOWED_CHANNELS = new Set([
  ...
  'cloud:status', 'cloud:connect', 'cloud:sync', 'cloud:disconnect', 'cloud:pull-meta',
...
// Deep-freeze helper — prevents renderer code from monkey-patching the API
// surface after contextBridge delivers it.
...
contextBridge.exposeInMainWorld('vaultAPI', deepFreeze({
  ...
  cloud: {
    ...
    pullMeta:   (cfg) => invoke('cloud:pull-meta', cfg),
    onProgress:      (cb) => ipcRenderer.on('cloud-progress',        (_, p) => cb(p)),
    onAutoSyncDone:  (cb) => ipcRenderer.on('cloud-auto-sync-done',  (_, p) => cb(p)),
    onAutoSyncFail:  (cb) => ipcRenderer.on('cloud-auto-sync-failed',(_, p) => cb(p)),
  },
  settings: {
  ...
  importExport: {
    import:           (filename, content, strategy) => invoke('entries:import', { filename, content, strategy }),
    exportEncrypted:  (password) => invoke('vault:export-encrypted', password),
    importEncrypted:  (opts)     => invoke('vault:import-encrypted', opts),
  },
  security: {
    integrityCheck: () => invoke('vault:integrity-check'),
    mixKeyfile:     (p) => invoke('keyfile:mix', p),
  },
  window: {
